...
class FaceDetector
{
    ...
    {
        $info = pathinfo($file);
        //$name = ($info['filename'] == '' ? $tglnow : $info['filename']).'.jpg';
        if ($this->face['w'] > 0) {
            $this->date = date('Y-m-d-H-i-s');
            imagejpeg($this->canvas, 'original/'.$this->date.'.jpg', 100);
        }
        if (is_resource($file)) {
            $this->canvas = $file;
        } elseif (is_file($file)) {
        ...
        if ($ratio != 0) {
            ...
            if ($this->face['w'] > 0) {
                ...
                $this->face['y'] *= $ratio;
                $this->face['w'] *= $ratio;
                // $tglnoww = date('Y-m-d-H-i-s');
                // imagejpeg($this->canvas, 'original/'.$tglnoww.'.jpg', 100);
            }
        } else {
    ...
    public function toJpeg($file_name = null)
    {
        ...
        // $name = 'temp/'.'cache'.$tglnow.'.jpg';
        $name = ($file_name == null ? $this->date : $file_name).'.jpg';
        // imagejpeg($this->canvas, 'temp/'.$tglnow.'.jpg', 100);

        imagejpeg($this->canvas, 'temp/'.$name, 100);
